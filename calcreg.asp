...
strPath=Server.MapPath("emplog.asp")
...
if ValidLogIn(strUserName, strPassword) then
	...
	response.cookies("loggedIN")="1111"
	response.cookies("Username").Expires=Date+1
    response.cookies("Password").Expires=Date+1
	response.cookies("loggedIN").Expires=Date+1
	response.redirect "calculator.asp"
	response.write "<a href='calculator.asp'>Click here if this page is not automatically refreshed.</a>"
    response.end
